...
class HGRU4Rec(object):
    ...
    def build_model(self,mode,step):
        self.s_state=tf.placeholder(tf.float32,(self.batch_size,self.s_size),'Session_State')
        self.u_state=tf.placeholder(tf.float32,(self.batch_size,self.u_size),'User_State')
        ...
        with tf.name_scope('Session_Reset'):
            with tf.variable_scope('Session_Reset',reuse=tf.AUTO_REUSE):
                _,updated_state=u_gru(tf.layers.dense(self.s_state,self.u_size),self.u_state)
                u_state = tf.where(self.flags,updated_state,self.u_state,name='user_update')
                s_gru_input=tf.where(self.flags,tf.layers.dense(u_state,self.s_size),self.s_state,name='input_state_for_s_gru')
                
        
        output,s_final_state=s_gru(x_embedded,s_gru_input)
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session() as sess:
                ...
                for n in range(num_epochs):
                    loss_sum=0
                    current_u_state=np.zeros([self.batch_size,self.u_size],dtype=np.float32)
                    current_s_state=np.zeros([self.batch_size,self.s_size],dtype=np.float32)
                    for i in range(len(inputs)):
                        ...
                        feeds={
                                ...
                                self.u_state:current_u_state,
                                self.s_state:current_s_state
                            }
...
if __name__=='__main__':
    ...
    model=HGRU4Rec(s_size=4,u_size=3,batch_size=3,s_dropout=0.5,u_dropout=0.5,len_items=item_len)
